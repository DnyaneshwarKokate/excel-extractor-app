...
def get_month_sheet_name(d_str):
    try:
        ...
        if month_name == 'February':
            month_name = 'Feburary' # Match Excel sheet spelling
        return f"{month_name} - {dt.year}"
    except:
        return "June - 2026"

def append_records(filepath, sheet_name, records):
    if not os.path.exists(filepath):
        return json.dumps({"success": False, "error": f"File not found: {filepath}"})
    ...
    try:
        with zipfile.ZipFile(filepath, 'r') as z:
            ...
            # Map sheet name to target xml file
            target_xml = None
            ...
            target_sheet_search = sheet_name.strip().lower()
            for s in sheets:
            ...
            if not target_xml:
                target_xml = 'xl/worksheets/sheet2.xml'
        ...
        # Existing Job Card Nos to prevent duplicate entry
        existing_jcs = set()
        next_row = 6
        ...
        for rec in records:
            ...
            d_id = add_ss(rec.get('partNumber', 'D11000849LB201R0A'))
        ...
        return json.dumps({"success": True, "addedRows": added_rows, "targetXml": target_xml, "backupFile": backup_file})
