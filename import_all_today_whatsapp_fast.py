...
print(f"Scanning {len(imgs)} WhatsApp images in parallel...")
...
def process_one_image(img_path):
    ...
    p_res = subprocess.run(['python3', '/Users/dnyaneshwarkokate/.gemini/antigravity/scratch/excel-extractor-app/ocr_parser.py', img_path], capture_output=True, text=True)
    try:
        rec = json.loads(p_res.stdout)
        text = rec.get('rawText', '')
    except Exception as e:
        return None

    # Job Card No
    jc_match = re.search(r'JC2627-(\d+)', text, re.IGNORECASE)
    jc_no = int(jc_match.group(1)) if jc_match else rec.get('jobCardNo', 0)
    
    # Date
    date_pos = text.find('Date:')
    d_val = rec.get('date', '20.06.2026')
    if date_pos != -1:
    ...
    p_num = p_num_match.group(1) if p_num_match else rec.get('partNumber', 'D11000849LB201R0A')
    
    # Part Name
    p_name = rec.get('partName', 'PANEL STICKER-1')
    p_name_match = re.search(r'Item Name:\s*(.+)', text, re.IGNORECASE)
    if p_name_match:
        p_name_candidate = p_name_match.group(1).strip()
        if 'BOM' in p_name_candidate:
            p_name_candidate = p_name_candidate.split('BOM')[0].strip()
        if len(p_name_candidate) > 3:
            p_name = p_name_candidate
    ...
    # PO No
    po_no = rec.get('poNo', '')
    po_match = re.search(r'(\d{10}\/?\d*|PUR\/\d+\/\w+|TA20\d+|5501\d+|5700\d+)', text)
    if po_match:
...
with ThreadPoolExecutor(max_workers=10) as executor:
    results = list(executor.map(process_one_image, imgs))

parsed_records = [r for r in results if r is not None]
print(f"Successfully extracted {len(parsed_records)} records.")

# Append to Excel starting at Row 27
ET.register_namespace('', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main')
ET.register_namespace('r', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships')
ET.register_namespace('mc', 'http://schemas.openxmlformats.org/markup-compatibility/2006')
ET.register_namespace('x14ac', 'http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac')

scratch_temp = '/Users/dnyaneshwarkokate/.gemini/antigravity/scratch/temp_all_import_fast.xlsx'
...
print(f"\nSUCCESSFULLY IMPORTED {len(added_summary)} RECORDS INTO EXCEL FROM ROW 27 TO ROW {next_row - 1}!")
